...
void hvl_reset_some_stuff( struct hvl_tune *ht )
{
  ...
  ht->ht_CurrentFrame = 0;  // счет фремов
...
// Добавьте новую функцию для получения текущего фрейма
uint32 hvl_GetCurrentFrame( struct hvl_tune *ht )
{
  return ht->ht_CurrentFrame;
}
