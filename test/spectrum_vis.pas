...
interface
 ...
 type TSpectrum = Class(TObject)
    ...
    public
     ...
     procedure Draw(Canvas: TCanvas; FFTData : TFFTData; X, Y : Integer); // Изменено: принимаем TCanvas
     ...
     procedure TSpectrum.Draw(Canvas: TCanvas; FFTData : TFFTData; X, Y : Integer);
       ...
       if FrmClear then begin
        ...
        VisBuff.Canvas.Pen.Color := PenColor;
         ...
         // Кроссплатформенное копирование буфера на канвас
         Canvas.CopyRect(Rect(X, Y, X + VisBuff.Width, Y + VisBuff.Height),
                        VisBuff.Canvas,
